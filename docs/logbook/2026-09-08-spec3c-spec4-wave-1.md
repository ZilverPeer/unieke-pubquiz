...
- 15:25 drift, #132 implementer: after the probe it went into app internals (`proxy.ts`, `instrumentation.ts`, `next.config.ts`, the Edge Runtime docs) instead of identifying the sender of the twin delivery. One message: webhook lists and delivery logs of both wp-env instances, then the mu-plugin triggers, no app-side logging.

## 2026-09-09 16:2x #132 root cause: WooCommerce's unsigned webhook ping

The implementer proved with WooCommerce's own delivery log and Action Scheduler that the shop sends one signed delivery per order update (200) and that the webhook secret matches the app's (compared as SHA-256, never printed). The orchestrator added a one-probe header log on the 401 path of the running app (reverted): the twin is `WC_Webhook::deliver_ping()`, user agent "WooCommerce/11.1.0 Hookshot", form-urlencoded body `webhook_id=1`, no signature. Our 401 keeps the webhook's `pending_delivery` flag set (`wp eval` confirms), so Woo re-pings with every delivery. Fix redirected to the receiver: `handleWebhook` answers 200 to an unsigned body matching `^webhook_id=\d+$` and nothing else; unit tests red first; the flag is verified cleared on the loop after the merge. Issue #132's premise (a second sender) was wrong; the brief's hypotheses A and B both ruled out.
