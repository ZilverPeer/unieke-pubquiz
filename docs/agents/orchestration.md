...
The overhead of a ticket (worktree, PR, two reviews, fix round, merge) is about 20 minutes whatever the diff size, so the rule cuts both ways. Small fixes (roughly under 20 lines, one file, no new behaviour, such as a seed tweak or removing a footer) are batched into one "fixes" issue and one PR, handled by one implementer and reviewed by a single empirical reviewer. The two-reviewer process is for tickets that add or change behaviour.
