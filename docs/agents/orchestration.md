...
6. The orchestrator prepares every review clone before the reviewer is dispatched: `git clone`, `npm ci`, and a copy of `.env.local` (a review clone without `SUPABASE_URL` and its key cannot `npm run build`). The permission system denies reviewers copying `.env.local`, and the PR 125 reviewer froze on four attempts (2026-09-09). The brief names the prepared clone and its port.
7. `npm run db:reset` drops the operator account. Erik recreates it (`npm run admin:operator -- operator@example.com <password>`) before any agent that logs in is dispatched; the orchestrator's own run is denied by the permission classifier because the password is on the command line (2026-09-09, #107 reseed stalled #131 and the PR 130 review).
8. When the loop is up from the main checkout, every `wp-env` command runs with the main checkout as working directory: wp-env keys its environment on the directory, so `npx wp-env run cli` from a worktree creates a second, empty environment under `~/.wp-env` instead of talking to the running shop (#132 implementer, 2026-09-09).
...
More rules from the spec 4/5 wave (2026-09-09):

- curl multipart does post plain server forms: the login form and inline `<form action={serverFunction}>` forms, using the exact `$ACTION_ID_<hash>` hidden field the rendered HTML carries. Only `useActionState` forms are out of reach.
- Integration test files run one at a time. Two files started concurrently raced on shared temp directories and produced a false failure (PR 126 review).
- A red-claim recipe that checks out master's version of a directory (`git checkout origin/master -- <dir>`) does not delete files that exist only on the branch; the recipe removes those explicitly or the red is not proven (PR 126 review).
- Captured HTTP responses are printed without headers, or with `Set-Cookie`, `Authorization` and any `eyJ` line filtered out first; before printing any captured output the reviewer counts key-like lines (`grep -c -E "eyJ|ck_|cs_"`) and prints only when the count is zero. Three agents leaked a cookie or token fragment into a report this wave; local stack only, but the rule is absolute.
- A documentation PR (a walkthrough, a runbook) is walked against a loop started from master, reading the document from the branch with `git show origin/<branch>:<path>`; the orchestrator starts that loop before dispatch, the reviewer never runs `loop:up` (PR 130).
