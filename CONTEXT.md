...
- **Delivery**: the backend uploads the Deliverables to Storage, then the `deliver` module attaches their URLs to the order's line item as `meta_data` via the WooCommerce REST API (there is no supported REST way to attach a per-order downloadable file to a line item, and product-level downloadable files would leak one customer's files to every order of the same product). A must-use shop plugin renders that `meta_data` as labelled download links in the completed-order email, the customer's order view, and My Account → Downloads, and hides the raw meta from the customer-facing item table. Once every Quiz of the order is delivered, the order is set to **`completed`**, which triggers WooCommerce's own completed-order email — no separate email service or template. Failed generation (see Generation failure policy) means the order stays `processing` and the operator is alerted via a private order note; nothing goes to the customer. A shop plugin holds any order containing a Pubquiz product at `processing` after payment, since WooCommerce would otherwise auto-complete a fully virtual/downloadable order before generation has run.
