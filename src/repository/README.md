...
- `transitionQuizStatus(quizId, to, options?)` -- moves a Quiz along `QUIZ_STATUS_TRANSITIONS` (`src/domain/orders.ts`); an edge not listed there throws `IllegalQuizTransitionError`. `options.failureReason` is stored when transitioning to `failed`; transitioning to `pending` always clears it.
- `recordDelivery(quizId, { compositionId, downloadToken })` -- sets `compositionId`, `downloadToken` and `deliveredAt` together with the transition to `delivered` (so a Quiz can only be `delivered` with all three set).
