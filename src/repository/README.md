...
`src/repository/admin/items.ts` (spec 4, ticket #88) is the Items admin area's own read/write module -- a plain module of functions taking the typed Supabase client, the same shape as `orders.ts`, plus a `createItemsAdminRepository(config)` factory. `listItems` lists Items with filters (kind, Category chain, Difficulty, missing-Locale, archived) and search, paginated; `getItem` reads one Item with both Locale translations (and, additive, its Music detail row when the Item is a Music Item -- ticket #91); `loadSubsubcategoryOptions` returns every Subsubcategory as a `{ id, path }` dropdown option ("Category / Subcategory / Subsubcategory", names in the requested Locale) for the create/edit form. `writeItemBase` (insert when `id` is `null`, otherwise update the mutable Subsubcategory/Difficulty fields -- `kind` is fixed after creation) and `writeTranslations` (upserts a translation row per complete Locale, deletes the row for any Locale left empty) are written to be reused as-is by the Picture and Music tickets; `createTextItem`/`updateTextItem` are this ticket's kind-specific callers of both. Not folded into `ContentRepository`: the admin area writes rows the sample/render/order code paths never touch, the same reasoning as `OrderRepository`'s own separation (see "Order repository" above).
