...
- `failure-reason.ts` -- `buildFailureReason`, a pure function building the plain-words failure text `quiz-job.ts` stores on a failed Quiz and passes to `deliverer.noteFailure` (spec 3c, ticket #84). See "State machine and retry policy" below.
...
The whole of step 1 and 2 -- the lookup, the transition, the order lookup, and generation itself -- runs inside one try/catch, so *any* error from any part of it (not just a failure from `generateQuiz`) is handled the same way, and pg-boss never dead-letters a job leaving a Quiz stuck. Three outcomes:
   ...
   - **Terminal failure** -- an unsatisfiable checkout configuration (`InvalidQuizConfigError`, mirrors `resolveSlotCategories`'s own distinct/at-most-8 Category pick checks, `src/sample/index.ts`, as defense in depth) or a sampler shortfall (`QuizShortfallError`, wrapping `generateQuiz`'s `{ok: false}` result). Either way the message is built by `buildFailureReason` (`failure-reason.ts`, spec 3c, ticket #84): a plain-words, multi-line English text naming the Quiz's id and its number within its order, the billing email, the Locale, the Round kind (text/picture/music, from the failing slot index) and Category for a shortfall, the requested Difficulty, how many Items (or Round slots, if no Category was left to assign at all) are missing, and a fixed "What to do" paragraph -- add that many Items and retry the Quiz with `npm run generate -- --retry-quiz <the Quiz's id>` (the actual id, so the operator can copy the command as-is), or refund the order in WooCommerce (a configuration error instead says a refund is the only way out). For example:
     ...
     What to do: add at least 4 picture Items to "Geschiedenis" (hard, nl) and retry this Quiz with `npm run generate -- --retry-quiz 3f9c9b8e-6b7a-4d1e-9c2a-8e7f1a2b3c4d`, or refund the order in WooCommerce.
     ```

     The Quiz moves to `failed` with that text as the reason, `deliverer.noteFailure` is called with it unchanged (it prepends its own `[pubquiz]` marker and the line item id, see `src/deliver/index.ts`), and the job completes successfully (pg-boss never retries a terminal failure).
