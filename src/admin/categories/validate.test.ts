...
describe("validateAddNode", () => {
  ...
  it("requires a valid parentId for a Subcategory", () => {
    ...
    expect(result).toEqual({ ok: false, errors: { parentId: "errors.invalidParent" } });
  });
  ...
  it("rejects an empty en name", () => {
    ...
    expect(result).toEqual({ ok: false, errors: { nameEn: "errors.nameRequired" } });
  });

  it("rejects a name over 120 characters", () => {
    const tooLong = "a".repeat(121);
    const result = validateAddNode({ level: "category", parentId: null, nameNl: tooLong, nameEn: "Sports" });
    expect(result).toEqual({ ok: false, errors: { nameNl: "errors.nameTooLong" } });
  });
  ...
  it("rejects an invalid level", () => {
    ...
    expect(result).toEqual({ ok: false, errors: { level: "errors.invalidLevel" } });
  });
  ...
  it("reports every field error at once, nothing written", () => {
    ...
    expect(result).toEqual({
      ok: false,
      errors: {
        parentId: "errors.invalidParent",
        nameNl: "errors.nameRequired",
        nameEn: "errors.nameRequired",
      },
    });
  });
});
...
describe("validateRenameNode", () => {
  ...
  it("rejects an invalid locale", () => {
    ...
    expect(result).toEqual({ ok: false, errors: { locale: "errors.invalidLocale" } });
  });

  it("rejects a non-numeric id", () => {
    const result = validateRenameNode({ level: "category", id: "abc", locale: "nl", name: "Sport" });
    expect(result).toEqual({ ok: false, errors: { id: "errors.invalidId" } });
  });

  it("rejects an empty name", () => {
    const result = validateRenameNode({ level: "category", id: "3", locale: "nl", name: "   " });
    expect(result).toEqual({ ok: false, errors: { name: "errors.nameRequired" } });
  });
});
