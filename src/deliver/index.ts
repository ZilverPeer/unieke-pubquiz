...
export interface Deliverer {
  /** Attach the Quiz's zip URL to its line item; complete the order once every Quiz of it is delivered. */
  deliverQuiz(input: { quizId: string; url: string }): Promise<void>;
...
export function createDeliverer(config: DelivererConfig, orderLookup: OrderLookup): Deliverer {
  ...
  return {
    async deliverQuiz({ quizId, url }) {
      ...
      // entry in place (by its WooCommerce meta id) instead of duplicating
      // it -- WooCommerce's order PUT has no upsert-by-key semantics for
      // meta_data (see README.md "How downloads are attached").
      ...
      const key = downloadMetaKey(sequence);
      const existing = existingMeta.find((meta) => meta.key === key);
      const metaDatum: WooMetaDatum = existing ? { id: existing.id, key, value: url } : { key, value: url };

      await client.put(`/wc/v3/orders/${wooOrderId}`, {
        line_items: [{ id: wooLineItemId, meta_data: [metaDatum] }],
      });
