...
- Each line item's config is parsed and validated: Locale/difficulty must be one of the known values, each Category pick (read in pick order, `pubquiz_category_1..8`, stopping at the first missing key) must be a real, distinct Category id (checked via `createCategoryIdLookup`, `src/repository/index.ts`). A line item that fails any of these still gets inserted (see "Interface gap" below) and is immediately transitioned to `failed` with the reason -- **and noted**, the same operator-alert order note every other `failed` path posts (see "Failed-line-item note" below); its siblings in the same order are unaffected.
