"use client";
/**
 * Picture Item bulk import form (spec 4, ticket #95). The Picture sibling
 * of ../import-form.tsx: two file inputs (`csv`, `zip`) instead of one, but
 * the same useActionState/row-report shape, reusing splitFieldErrors and
 * RowErrorTable from ../row-error-table.tsx rather than duplicating them.
 *
 * Two translators: `t` scoped to "pictureImport" for this page's own copy;
 * `tReport` scoped to "itemsImport" so RowErrorTable's report.* labels are
 * reused, not duplicated (ticket #95 decision); `tMessage` unscoped, to
 * resolve the fully-qualified message keys a row's `message` carries
 * ("items.errors.X", "pictureItems.errors.file.X", "pictureImport.errors.X").
 */
import { useActionState } from "react";
import { useTranslations } from "next-intl";
import type { ActionResult } from "@/admin/forms";
import { pictureCsvColumnForField } from "@/admin/items/import-picture-csv";
...
export function PictureImportForm() {
  const t = useTranslations("pictureImport");
  ...
  async function submit(_previous: FormState, formData: FormData): Promise<FormState> {
    return importPictureItems(formData);
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <RowErrorTable rowErrors={rowErrors} columnForField={pictureCsvColumnForField} t={tReport} tMessage={tMessage} />
    </div>
  );
}
